...
public class ObjectReuseExtendedMeasurementSource
        extends RichParallelSourceFunction<ExtendedMeasurement> {
    ...
    public void open(final Configuration parameters) {
        ...
        getRuntimeContext()
                .getMetricGroup()
                .gauge("sourceTemperatureUSmin", new MeanGauge.MinGauge(sourceTemperatureUS));
        getRuntimeContext()
                .getMetricGroup()
                .gauge("sourceTemperatureUSmax", new MeanGauge.MaxGauge(sourceTemperatureUS));
    }
    ...
    private void initSensors() {
        final SplittableRandom rnd = new SplittableRandom();
        final Sensor.SensorType[] sensorTypes = Sensor.SensorType.values();
        ...
        for (int i = 0; i < NUM_SENSORS; ++i) {
            ...
            final Sensor.SensorType sensorType = sensorTypes[(i / 2) % sensorTypes.length];
    ...
    private double randomInitialMeasurementValue(
            SplittableRandom rnd, Sensor.SensorType sensorType) {
        switch (sensorType) {
            case Temperature:
                // -10°C - 35°C
                return rnd.nextInt((UPPER_TEMPERATURE_CELCIUS - LOWER_TEMPERATURE_CELCIUS) * 10)
                                / 10.0
    ...
    private ExtendedMeasurement randomEvent(SplittableRandom rnd) {
        ...
        final double value =
                randomChangeMeasurementValue(
                        rnd, sensor.getSensorType(), location, lastValue[randomIdx]);
        ...
        return new ExtendedMeasurement(
                new Sensor(sensor.getSensorId(), sensor.getVendorId(), sensor.getSensorType()),
                new Location(location.getLongitude(), location.getLatitude(), location.getHeight()),
                measurement);
    }
    ...
    private double randomChangeMeasurementValue(
        ...
        switch (sensorType) {
            case Temperature:
                value =
                        newValueWithinBounds(
                                lastValue,
                                change,
                                LOWER_TEMPERATURE_CELCIUS,
                                UPPER_TEMPERATURE_CELCIUS);
    ...
    /**
     * Returns either <code>lastValue + change</code> (if within the given bounds) or <code>
     * lastValue - change</code> (otherwise).
     */
    private static double newValueWithinBounds(
            double lastValue, double change, double lowerLimit, double upperLimit) {
        double value;
